...
The LoRA configurations are defined in `cosmos_predict2/experiments/base/cosmos_h_surgical_assets_lora.py`.

Two configurations are provided:
- `predict2_lora_training_2b_cosmos_h_surgical_assets_txt` - For text caption format (`.txt` files in `metas/` directory)
- `predict2_lora_training_2b_cosmos_h_surgical_assets_json` - For JSON caption format (`.json` files in `captions/` directory)
...
torchrun --nproc_per_node=8 scripts/train.py \
  ...
  experiment=predict2_lora_training_2b_cosmos_h_surgical_assets_txt
```
...
torchrun --nproc_per_node=8 scripts/train.py \
  --config=cosmos_predict2/_src/predict2/configs/video2world/config.py -- \
  experiment=predict2_lora_training_2b_cosmos_h_surgical_assets_json
```

Checkpoints are saved to:
- Text format: `${IMAGINAIRE_OUTPUT_ROOT}/cosmos_h_surgical_predict/lora/2b_cosmos_h_surgical_assets_lora_txt/checkpoints`
- JSON format: `${IMAGINAIRE_OUTPUT_ROOT}/cosmos_h_surgical_predict/lora/2b_cosmos_h_surgical_assets_lora_json/checkpoints`
...
CHECKPOINTS_DIR=${IMAGINAIRE_OUTPUT_ROOT:-/tmp/imaginaire4-output}/cosmos_h_surgical_predict/lora/2b_cosmos_h_surgical_assets_lora_txt/checkpoints
