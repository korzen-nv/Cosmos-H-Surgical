...
def get_sampler(dataset) -> DistributedSampler:
    ...
    print(f"Sampler with data loader seed: {DATA_LOADER_SEED}")
...
class SingleViewTransferDatasetJSON(Dataset):
    ...
    Dataset structure:
        dataset_dir/
            ...
            └── video2.json
            ├── video1.depth.mp4  (for depth)
            └── video1.seg.mp4  (for seg)
    ...
    def _load_control_data(self, video_path: str, frame_ids: list[int]) -> dict[str, Any] | None:
        ...
        try:
            if self.ctrl_type == "seg":
                ctrl_path = video_path.replace(".mp4", ".seg.mp4")
                ...
                # Load depth video
                ctrl_path = video_path.replace(".mp4", ".depth.mp4")
                if not os.path.exists(ctrl_path):
                    raise FileNotFoundError(f"Control input file not found: {ctrl_path}")
                vr = VideoReader(ctrl_path, ctx=cpu(0))
                if len(vr) < frame_ids[-1] + 1:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        """
        max_retries = 1  # Try up to 10 different videos
        ...
        for retry in range(max_retries):
            ...
            try:
                ...
                caption = self._load_json_caption(caption_path)
                ...
                ctrl_data = self._load_control_data(video_path, frame_ids)
...
if __name__ == "__main__":
    ...
    Usage:
        PYTHONPATH=. python cosmos_transfer2/_src/transfer2/datasets/local_datasets/singleview_dataset_json.py
    """
    import sys

    # Example dataset with edge control (computed on-the-fly)
    dataset = SingleViewTransferDatasetJSON(
        dataset_dir="/healthcareeng_monai/datasets/BSA_HighFPS,/healthcareeng_monai/datasets/BSA_HighFPS",
        json_path="/healthcareeng_monai/datasets/BSA_HighFPS/aspiration_resized_93x1280x704_16fps.json,/healthcareeng_monai/datasets/BSA_HighFPS/aspiration_resized_93x1280x704_16fps.json",
        enlarged_factor="1.0,2.0",
        num_frames=93,
        video_size=(704, 1280),
        resolution="720",
        caption_format="json",
    ...
    for idx in indices:
        ...
        try:
            ...
            log.info(f"  Control input shape: {data['control_input_seg'].shape}")
