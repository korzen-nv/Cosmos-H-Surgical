# SPDX-FileCopyrightText: Copyright (c) 2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""
Post-training configurations for Cosmos-H-Surgical-Transfer with local single-view datasets.

These configurations demonstrate how to post-train Cosmos-H-Surgical-Transfer models on your own data.
They inherit from the base 2B control experiment and override dataset and checkpoint settings.

Example usage:
    torchrun --nproc_per_node=8 -m scripts.train \\
        --config=cosmos_transfer2/_src/transfer2/configs/vid2vid_transfer/config.py \\
        -- experiment=cosmos_h_surgical_transfer_singleview_posttrain_edge
"""

import os

from hydra.core.config_store import ConfigStore

from cosmos_transfer2._src.imaginaire.utils.checkpoint_db import get_checkpoint_path
from cosmos_transfer2.config import DEFAULT_BASE_EXPERIMENT, MODEL_CHECKPOINTS, ModelKey, ModelVariant

# Default model keys and checkpoints for each control type
DEFAULT_EDGE_MODEL_KEY = ModelKey(variant=ModelVariant.EDGE)
DEFAULT_DEPTH_MODEL_KEY = ModelKey(variant=ModelVariant.DEPTH)
DEFAULT_SEG_MODEL_KEY = ModelKey(variant=ModelVariant.SEG)
DEFAULT_VIS_MODEL_KEY = ModelKey(variant=ModelVariant.VIS)

EDGE_CHECKPOINT = MODEL_CHECKPOINTS[DEFAULT_EDGE_MODEL_KEY]
DEPTH_CHECKPOINT = MODEL_CHECKPOINTS[DEFAULT_DEPTH_MODEL_KEY]
SEG_CHECKPOINT = MODEL_CHECKPOINTS[DEFAULT_SEG_MODEL_KEY]
VIS_CHECKPOINT = MODEL_CHECKPOINTS[DEFAULT_VIS_MODEL_KEY]


_CHECKPOINT_CONFIG = dict(
    save_iter=50,
    load_training_state=False,
    strict_resume=False,
    load_from_object_store=dict(enabled=False),
    save_to_object_store=dict(enabled=False),
)

_TRAINER_CONFIG = dict(
    logging_iter=5,
    max_iter=20_000,
    straggler_detection=dict(enabled=False),  # Disable for local training
    callbacks=dict(
        heart_beat=dict(save_s3=False),
        iter_speed=dict(hit_thres=50, save_s3=False),
        device_monitor=dict(save_s3=False),
        every_n_sample_reg=dict(save_s3=False, every_n=50000),
        every_n_sample_ema=dict(save_s3=False, every_n=50000),
        wandb=dict(save_s3=False),
        wandb_10x=dict(save_s3=False),
        dataloader_speed=dict(save_s3=False),
        frame_loss_log=dict(save_s3=False),
    ),
)

_SCHEDULER_CONFIG = dict(
    f_max=[1.0],
    f_min=[0.2],
    warm_up_steps=[1_000],
    cycle_lengths=[100_000],
    verbosity_interval=5,
)

_OPTIMIZER_CONFIG = dict(
    lr=5e-5,
    weight_decay=0.001,
)

WORLD_SIZE = int(os.environ.get("WORLD_SIZE", "1"))
_MODEL_PARALLEL_CONFIG = dict(
    context_parallel_size=1,  # WORLD_SIZE if WORLD_SIZE < 8 else 8,
)

# =============================================================================
# Post-training with Edge Control (2B Model)
# =============================================================================

cosmos_h_surgical_transfer_singleview_posttrain_edge = dict(
    defaults=[
        DEFAULT_BASE_EXPERIMENT,
        {"override /data_train": "surgical_singleview_train_data_edge"},
    ],
    job=dict(
        project="cosmos_h_surgical_transfer_posttrain",
        group="local_single_view",
        name="cosmos_h_surgical_transfer_singleview_posttrain_edge",
    ),
    checkpoint=dict[str, int | dict[str, bool]](
        load_path=get_checkpoint_path(EDGE_CHECKPOINT.s3.uri),
        **_CHECKPOINT_CONFIG,
    ),
    model=dict(
        config=dict(
            hint_keys="edge",
            base_load_from=None,  # Disable base model loading (already loading from checkpoint.load_path)
        ),
    ),
    dataloader_train=dict(
        dataset=dict(
            control_input_type="edge",
        ),
    ),
    trainer=_TRAINER_CONFIG,
    scheduler=_SCHEDULER_CONFIG,
    optimizer=_OPTIMIZER_CONFIG,
    model_parallel=_MODEL_PARALLEL_CONFIG,
)


# =============================================================================
# Post-training with Depth Control (2B Model)
# =============================================================================

cosmos_h_surgical_transfer_singleview_posttrain_depth = dict(
    defaults=[
        DEFAULT_BASE_EXPERIMENT,
        {"override /data_train": "surgical_singleview_train_data_depth"},
    ],
    job=dict(
        project="cosmos_h_surgical_transfer_posttrain",
        group="local_single_view",
        name="cosmos_h_surgical_transfer_singleview_posttrain_depth",
    ),
    checkpoint=dict(
        load_path=get_checkpoint_path(DEPTH_CHECKPOINT.s3.uri),
        **_CHECKPOINT_CONFIG,
    ),
    model=dict(
        config=dict(
            hint_keys="depth",
            base_load_from=None,  # Disable base model loading (already loading from checkpoint.load_path)
        ),
    ),
    dataloader_train=dict(
        dataset=dict(
            control_input_type="depth",
        ),
    ),
    trainer=_TRAINER_CONFIG,
    scheduler=_SCHEDULER_CONFIG,
    optimizer=_OPTIMIZER_CONFIG,
    model_parallel=_MODEL_PARALLEL_CONFIG,
)


# =============================================================================
# Post-training with Segmentation Control (2B Model)
# =============================================================================

cosmos_h_surgical_transfer_singleview_posttrain_seg = dict(
    defaults=[
        DEFAULT_BASE_EXPERIMENT,
        {"override /data_train": "surgical_singleview_train_data_seg"},
    ],
    job=dict(
        project="cosmos_h_surgical_transfer_posttrain",
        group="local_single_view",
        name="cosmos_h_surgical_transfer_singleview_posttrain_seg",
    ),
    checkpoint=dict(
        load_path=get_checkpoint_path(SEG_CHECKPOINT.s3.uri),
        **_CHECKPOINT_CONFIG,
    ),
    model=dict(
        config=dict(
            hint_keys="seg",
            base_load_from=None,  # Disable base model loading (already loading from checkpoint.load_path)
        ),
    ),
    dataloader_train=dict(
        dataset=dict(
            control_input_type="segcolor",
        ),
    ),
    trainer=_TRAINER_CONFIG,
    scheduler=_SCHEDULER_CONFIG,
    optimizer=_OPTIMIZER_CONFIG,
    model_parallel=_MODEL_PARALLEL_CONFIG,
)


# =============================================================================
# Post-training with Vis (Blur) Control (2B Model)
# =============================================================================

cosmos_h_surgical_transfer_singleview_posttrain_vis = dict(
    defaults=[
        DEFAULT_BASE_EXPERIMENT,
        {"override /data_train": "surgical_singleview_train_data_vis"},
    ],
    job=dict(
        project="cosmos_h_surgical_transfer_posttrain",
        group="local_single_view",
        name="cosmos_h_surgical_transfer_singleview_posttrain_vis",
    ),
    checkpoint=dict(
        load_path=get_checkpoint_path(VIS_CHECKPOINT.s3.uri),
        **_CHECKPOINT_CONFIG,
    ),
    model=dict(
        config=dict(
            hint_keys="vis",
            base_load_from=None,  # Disable base model loading (already loading from checkpoint.load_path)
        ),
    ),
    dataloader_train=dict(
        dataset=dict(
            control_input_type="vis",
        ),
    ),
    trainer=_TRAINER_CONFIG,
    scheduler=_SCHEDULER_CONFIG,
    optimizer=_OPTIMIZER_CONFIG,
    model_parallel=_MODEL_PARALLEL_CONFIG,
)


# =============================================================================
# Register all experiments
# =============================================================================

cs = ConfigStore.instance()

for _item in [
    cosmos_h_surgical_transfer_singleview_posttrain_edge,
    cosmos_h_surgical_transfer_singleview_posttrain_depth,
    cosmos_h_surgical_transfer_singleview_posttrain_seg,
    cosmos_h_surgical_transfer_singleview_posttrain_vis,
]:
    _name: str = _item["job"]["name"]  # pyrefly: ignore
    cs.store(
        group="experiment",
        package="_global_",
        name=_name,
        node=_item,
    )
