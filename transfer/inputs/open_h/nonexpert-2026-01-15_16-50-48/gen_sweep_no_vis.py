#!/usr/bin/env python3
"""Generate 64 multicontrol weight sweep specs with vis=0.0 (4^3 factorial)."""
...
def main():
    out_path = Path(__file__).parent / "ep043_sweep_no_vis.jsonl"
    specs = []

    for d_w, e_w, s_w in itertools.product(WEIGHTS, repeat=3):
        v_w = 0.0
    ...
    assert len(specs) == 64, f"Expected 64, got {len(specs)}"

    names = [s["name"] for s in specs]
    assert len(set(names)) == 64, "Duplicate names found!"
